...
const riderEmail = (digits: string) => `rider.${digits}@ironcloud.app`;
...
const ensureRiderAuthUser = async (
  admin: AdminClient,
  digits: string,
  existingProfileId?: string,
): Promise<{ user: User; isNewUser: boolean }> => {
  const e164 = toE164(digits);
  const email = riderEmail(digits);
  ...
  const created = await admin.auth.admin.createUser({
    ...
    email,
    email_confirm: true,
    user_metadata: { phone: digits, role: 'rider' },
  });

  if (created.data.user) {
    await admin
      .from('profiles')
      .update({ phone: digits, role: 'rider' })
  ...
  throw created.error ?? new Error('Failed to create rider auth user.');
};

const ensureRiderRow = async (
  admin: AdminClient,
  userId: string,
): Promise<{ isActive: boolean; isNewRider: boolean }> => {
  ...
  if (existing) {
    return { isActive: Boolean(existing.is_active), isNewRider: false };
  }
  ...
  if (error) {
    ...
    const { data: again } = await admin
      ...
      .maybeSingle();
    if (again) {
      return { isActive: Boolean(again.is_active), isNewRider: false };
    }
    throw error;
  }

  return { isActive: false, isNewRider: true };
};

const mintSession = async (admin: AdminClient, user: User, digits: string) => {
  const email = user.email ?? riderEmail(digits);
...
export async function POST(request: Request) {
  try {
    ...
    if (!authkey) {
      return json({ error: 'MSG91_AUTHKEY is not configured on the server.' }, 500);
    }

    const { url, serviceRoleKey, missing } = getServerSupabaseEnv();
    if (missing.length > 0) {
      return json({ error: `Missing server env: ${missing.join(', ')}` }, 500);
    }
    ...
    if (body.phone) {
      ...
      if (digits && clientDigits !== digits) {
        return json({ error: 'Phone number does not match verified OTP.' }, 401);
      }
      if (!digits) digits = clientDigits;
    }

    if (!digits || digits.length !== 10) {
      return json({ error: 'Could not resolve a verified phone number.' }, 401);
    }
    ...
    const { user, isNewUser } = await ensureRiderAuthUser(admin, digits, profile?.id);
    ...
    const { isActive, isNewRider } = await ensureRiderRow(admin, user.id);
    const session = await mintSession(admin, user, digits);

    return json({
      success: true,
      isNewUser: isNewUser || isNewRider,
      isActive,
      phone: digits,
      ...session,
    });
  } catch (err) {
    console.error('[msg91-session-rider]', err);
